...
// Ensure import for defaultTpPjok is present
if (!code.includes('defaultTpPjok')) {
  code = code.replace(
...
// Update the smart parser logic for 6 mapels
const newLogic = `
    ...
    if (!isPancasila && !isInggris && !isIndonesia && !isMatematika && !isIpas && !isPjok) {
      showNotif("Maaf, muat TP otomatis saat ini baru tersedia untuk mapel: Pancasila, B. Inggris, B. Indonesia, Matematika, IPAS, dan PJOK.", "error");
      return;
    }
