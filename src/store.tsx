...
export function AppProvider({ children }: { children: React.ReactNode }) {
    ...
    const handleStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEY && e.newValue) {
        try {
          ...
          if (JSON.stringify(stateRef.current) !== e.newValue) {
            setState(newState);
          }
        } catch (err) {}
      }
  ...
  useEffect(() => {
    ...
    // Kaidah Refresh Murni:
    // Setiap kali browser di-refresh, tarik data profil dasar lalu segera tarik payload terbaru dari awan.
    fetchLatestBaseline().finally(() => {
      forceSyncFromCloud();
    });
  }, []);
  ...
  const updateSekolah = (data: Partial<AppState['sekolah']>) => {
    setState((prev) => ({
      ...prev,
      sekolah: { ...prev.sekolah, ...data, timestamp: Date.now() },
    }));
  };
