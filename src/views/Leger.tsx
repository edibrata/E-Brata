...
export default function Leger() {
  ...
  return (
    <div className="w-full flex flex-col gap-3 h-[calc(100vh-8rem)] animate-in fade-in duration-200">
      {/* Block 1: Header & Toolbar */}
      <div className="bg-white p-3 sm:px-4 sm:py-3 rounded-2xl shadow-xs border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
        <div className="flex items-center gap-2">
          <span className="p-1.5 bg-indigo-50 text-indigo-700 rounded-lg border border-indigo-100/80">
            <Table size={18} />
          </span>
          <h2 className="text-sm font-bold text-slate-800">Leger Nilai</h2>
        </div>

        <Tooltip content="Unduh Rekapitulasi Leger Nilai Rapor ke Excel (.xlsx)" position="bottom">
          <button
            type="button"
            onClick={handleExportLeger}
            className="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-600 text-white hover:bg-emerald-700 transition flex items-center gap-1.5 cursor-pointer shadow-xs self-start sm:self-auto"
          >
            <Download size={14} />
      ...
      <div className="bg-white p-3 sm:p-4 rounded-2xl shadow-xs border border-slate-200 flex flex-col flex-1 overflow-hidden min-h-0">
        ...
        <div className="mt-3 text-[11px] text-slate-500 flex items-center justify-between shrink-0">
          <p>
