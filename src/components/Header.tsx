  ...
  return (
    <header className="main-header bg-gradient-to-r from-blue-800 to-blue-600 text-white shadow-md h-14 flex items-center px-4 md:px-6 justify-between sticky top-0 z-30 shrink-0">
      <div className="flex items-center gap-2 sm:gap-4">
        <Tooltip content="Buka / Tutup Menu Navigasi Samping" position="bottom">
          <button 
            onClick={toggleSidebar}
            className="text-white focus:outline-none p-1.5 rounded-md hover:bg-blue-800 transition-colors cursor-pointer"
        ...
        <div>
          ...
          <div className="text-white font-bold text-[11px] sm:text-sm md:text-base flex items-center gap-1.5">
            <span className="hidden sm:inline">Aplikasi Rapor Kurikulum Merdeka</span><span className="sm:hidden">E-Rapor KM</span>
            <Tooltip content="Profil & Portofolio Pengembang Aplikasi (Edi Brata)" position="bottom">
              <span 
      ...
      <div className="flex items-center gap-2 sm:gap-4 md:gap-6">
        ...
        <Tooltip 
          content={syncStatus === 'synced' ? 'Semua perubahan tersimpan aman di Cloud Database' : syncStatus === 'syncing' ? 'Sedang menyelaraskan data ke Cloud...' : 'Koneksi terputus, periksa internet Anda!'} 
          position="bottom"
        >
          <div className="flex items-center gap-1.5 px-2 sm:px-3 py-1 text-[11px] font-medium rounded-full bg-blue-800 border border-blue-600 cursor-default">
            {syncStatus === 'synced' && (
              <>
